...
/**
 * This is a copy of the original PdoSessionHandler found in Symfony but due to a limitation in sqlsrv we
 * need to copy this entire class because Symfony uses a lot of private properties which prevents
 * us from making only the changes we need, changes we had to do:
 *
 * - base64 encode/decode data being saved
 * - update write() so sqlsrv does not break (https://github.com/symfony/symfony/pull/18501/files)
 *
 *
    ...
    private function getMergeSql()
    {
        switch ($this->driver) {
            ...
            case 'oci':
                ...
                return "MERGE INTO $this->table USING DUAL ON ($this->idCol = ?) ".
                "WHEN NOT MATCHED THEN INSERT ($this->idCol, $this->dataCol, $this->lifetimeCol, $this->timeCol) VALUES (?, ?, ?, ?) ".
                "WHEN MATCHED THEN UPDATE SET $this->dataCol = ?, $this->lifetimeCol = ?, $this->timeCol = ?";
            case 'sqlsrv' === $this->driver && version_compare($this->pdo->getAttribute(\PDO::ATTR_SERVER_VERSION), '10', '>='):
                // MERGE is only available since SQL Server 2008 and must be terminated by semicolon
                // It also requires HOLDLOCK according to http://weblogs.sqlteam.com/dang/archive/2009/01/31/UPSERT-Race-Condition-With-MERGE.aspx
                return "MERGE INTO $this->table WITH (HOLDLOCK) USING (SELECT 1 AS dummy) AS src ON ($this->idCol = ?) ".
                "WHEN NOT MATCHED THEN INSERT ($this->idCol, $this->dataCol, $this->lifetimeCol, $this->timeCol) VALUES (?, ?, ?, ?) ".
                "WHEN MATCHED THEN UPDATE SET $this->dataCol = ?, $this->lifetimeCol = ?, $this->timeCol = ?;";
            case 'sqlite':
